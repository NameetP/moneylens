    ...
    <div>
      ...
      <div className="flex items-center justify-center mb-8">
        <div className="relative" style={{ width: 240, height: 240 }}>
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
            <span className="text-xs text-[#A8A29E] font-medium">Total</span>
            <AnimatedNumber
              value={data.totalSpend}
              prefix="AED "
              className="text-xl font-bold text-[#1C1917] tabular-nums"
            />
          </div>
        </div>
      </div>
      ...
      <div className="space-y-2">
        {displayCategories.map((cat) => (
          <div
            key={cat.name}
            className="flex items-center gap-3 px-4 py-3 rounded-2xl bg-white border-2 border-[#E7E5E4] hover:border-[#D6D3D1] hover:shadow-sm transition-all"
          ...
          >
            ...
            />
            <span className="text-sm font-semibold text-[#1C1917] w-28 shrink-0">
              {cat.name}
            </span>
            ...
            <span className="text-sm font-mono text-[#1C1917] tabular-nums whitespace-nowrap">
              AED {cat.amount.toLocaleString("en-AE")}
            </span>
            <span className="text-xs text-[#A8A29E] tabular-nums w-16 text-right shrink-0">
              {cat.percentage}%
            </span>
          </div>
        ))}
