...
export function SpendingBreakdown({ data }: Props) {
  // Show top 7 categories individually, merge rest into "Everything Else"
    ...
    <div>
      ...
      <div className="space-y-2">
        {displayCategories.map((cat) => (
          <div
            key={cat.name}
            className="flex items-center gap-3 px-4 py-3 rounded-xl bg-white border border-[#e5e5e5] shadow-sm"
          >
            <div
              className="w-3 h-3 rounded-full shrink-0"
              style={{ backgroundColor: cat.color }}
            />
            <span className="text-sm font-medium text-[#0a0a0a] w-28 shrink-0">
              {cat.name}
            </span>
            ...
            <span className="text-sm font-mono text-[#0a0a0a] tabular-nums whitespace-nowrap">
              AED {cat.amount.toLocaleString("en-AE")}
            </span>
            <span className="text-xs text-[#a3a3a3] tabular-nums w-16 text-right shrink-0">
              {cat.percentage}%
            </span>
          </div>
        ))}
